...
def add_user_info():
    ...
    u_i = User_Info(
        address="karachi2", postalcode="751002", about="here is me2", user_id=user_data
    )
    user_data.info = u_i
...
def get_user():
    v = User.query.filter(
        User.info != None
    ).all()  # query for removing empty relationship
    v = User.query.filter(
        and_(User.info != None, User.id == 2)
    ).all()  # query for removing empty relationship and using and operation get_user_and_userinfo_by_join()
